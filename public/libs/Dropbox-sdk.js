(function (global, factory) {
  ...
  function createBrowserSafeString(toBeConverted) {
    var convertedString = toBeConverted
      .toString("base64")
      .replace(/\+/g, "-")
      .replace(/\//g, "_")
      .replace(/=/g, "");
  ...
  var DropboxResponseError = /*#__PURE__*/ (function (_Error) {
    ...
    function DropboxResponseError(status, headers, error) {
      ...
      _this = _super.call(
        this,
        "Response failed with a ".concat(status, " code")
      );
    ...
    _createClass(DropboxAuth, [
        ...
        value: function generatePKCECodes() {
          ...
          if (isBrowserEnv()) {
            return crypto.subtle
              .digest("SHA-256", codeData)
              .then(function (digestedHash) {
                var typedArray = new Uint8Array(digestedHash);
                var base64String = btoa(typedArray);
                codeChallenge = createBrowserSafeString(base64String).substr(
                  0,
                  128
                );
      ...
      },
      {
        ...
        value: function getAuthenticationUrl(redirectUri, state) {
          ...
          if (tokenAccessType && !TokenAccessTypes.includes(tokenAccessType)) {
            throw new Error(
              "Token Access Type must be legacy, offline, or online"
            );
          }
          ...
          if (authType === "code") {
            authUrl = ""
              .concat(baseUrl, "?response_type=code&client_id=")
              .concat(clientId);
          } else {
            authUrl = ""
              .concat(baseUrl, "?response_type=token&client_id=")
              .concat(clientId);
          }
      ...
      },
      {
        ...
        value: function refreshAccessToken() {
          ...
          refreshUrl += "?grant_type=refresh_token&refresh_token=".concat(
          ...
          return this.fetch(refreshUrl, fetchOptions)
            ...
            .then(function (res) {
              ...
              _this3.setAccessTokenExpiresAt(
                getTokenExpiresAtDate(res.result.expires_in)
              );
            });
        },
        /**
      ...
      },
      {
        ...
        value: function authenticateWithCordova(
          successCallback,
          errorCallback
        ) {
          var redirectUrl =
            ...
            function onLoadStop(event) {
              ...
              if (errorIndex > -1) {
                ...
                if (tokenIndex > -1) {
                  ...
                  var accessToken = event.url.substring(
                    tokenIndex,
                    tokenTypeIndex
                  );
            ...
            .then(function () {
              ...
              switch (auth) {
                case APP_AUTH:
                  ...
                  authHeader = b64(
                    ""
                      .concat(_this.auth.clientId, ":")
                      .concat(_this.auth.clientSecret)
                  );
                  fetchOptions.headers.Authorization = "Basic ".concat(
                    authHeader
                  );
                  break;
                ...
                case TEAM_AUTH:
                case USER_AUTH:
                  fetchOptions.headers.Authorization = "Bearer ".concat(
